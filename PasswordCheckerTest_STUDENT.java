...
public class PasswordCheckerTest_STUDENT {
	...
	public void setUp() throws Exception {
		String[] p = {"667788AA!", "roger3YYY$", "5Sal#", "gertrude22", "september30", "b3cDe", 
				"OrangesxxxYYzz#", "aa11Bb", "RilotRroject", "BBBbb@123"};
	...
	{
		try{
			assertTrue(PasswordCheckerUtility.isValidPassword​("abc456"));
	...
	{
		try{
			assertTrue(PasswordCheckerUtility.isValidPassword​("1234ABC"));
	...
	{
		try{
			assertEquals(true,PasswordCheckerUtility.isValidPassword​("5678!bBBB"));
	...
	{
		try{
			assertTrue(PasswordCheckerUtility.isValidPassword​("abcdABC"));
	...
	{
		try{
			assertEquals(true,PasswordCheckerUtility.isValidPassword​("excellentPWD2"));
		}
	...
	public void testInvalidPasswords() {
		...
		assertEquals(scan.next(), "667788AA!");
		String nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("lowercase"));
		
		scan = new Scanner(results.get(1));  
		assertEquals(scan.next(), "roger3YYY$");
		...
		assertEquals(scan.next(), "gertrude22");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("uppercase"));
		
		scan = new Scanner(results.get(4));  
		assertEquals(scan.next(), "september30");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("uppercase") );
		...
		assertEquals(scan.next(), "b3cDe");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("long") );
		
		scan = new Scanner(results.get(6));  
		assertEquals(scan.next(), "OrangesxxxYYzz#");
		nextResults = scan.nextLine().toLowerCase();
		...
		assertEquals(scan.next(), "RilotRroject");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("digit") );
		
		scan = new Scanner(results.get(9));  
		assertEquals(scan.next(), "BBBbb@123");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("sequence") );
	}
